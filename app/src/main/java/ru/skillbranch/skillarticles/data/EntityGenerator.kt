...
private val commentsContent = listOf(
    "Nice.Thanks for sharing. Next you asked? We need to add math/scientific notations to TextView and also draw curvy (and straight, of course) microeconomics graphs e.g cost curve, demand and supply curve etc. ItвЂ™d be nice if you can write about it for your next piece.",
...
A lot has already been written about the basics (starting with the very good documentation) so I wonвЂ™t cover them here. Instead I would like to focus on important pitfalls that are mostly undocumented and rarely discussed and may cause issues in your applications if you miss them. In this first article, IвЂ™ll talk about our beloved Fragments.
...
Your first attempt may be to simply move the observer subscription to `onCreate()` instead of `onViewCreated()`. But it wonвЂ™t work as expected without adding more boilerplate code: `LiveData` keeps track of which result has been delivered to which observer, and **it wonвЂ™t deliver the latest result again to a new view hierarchy because the observer hasnвЂ™t changed.** This means you have to manually check for a latest result and bind it to the new view hierarchy in `onViewCreated()` which is not very elegant:
...
Note that the binding code needs to be called at two different places, so you canвЂ™t just write an inline anonymous observer. And finally, there is no way with the current LiveData API to differentiate a null result from no result for the current value, so itвЂ™s better not to return any null result (for instance in case of error) when using this solution, which overall is probably the worst one.
### Manually unsubscribing the observer in onDestroyView()
This is a bit better than the first solution but you still canвЂ™t use an inline anonymous observer. You must declare it as a final field, subscribe it in `onViewCreated()` and also not forget to unsubscribe it in `onDestroyView()`, so there is still boilerplate code and room for errors.
...
One of the main benefits of using `LiveData` being that it takes care of unsubscribing the observer for you, itвЂ™s a pity that it has to be done manually in this case. Keep reading, we can still do better.
### Resetting an existing observer
ItвЂ™s not actually required to unsubscribe the current observer precisely in `onDestroyView()`, because it will eventually be unsubscribed automatically in `onDestroy()`. WhatвЂ™s important is that itвЂ™s unsubscribed **before an identical one is subscribed** in `onViewCreated()`, in order to avoid duplicates. Therefore, another valid solution is to unsubscribe right before subscribing, using for example this Kotlin extension function:
...
The fragment code is now less error-prone because there is one less callback to add, but we still need to declare the observer instance as a final field and reuse it, or else unsubscription will silently fail. Despite this, itвЂ™s probably my personal favorite solution for a quick workaround.
...
> Note: `onViewCreated()` wonвЂ™t be called if `onCreateView()` returns `null`, so the custom `LifecycleOwner` wonвЂ™t be created for headless fragments and `getViewLifecycleOwner()` will also return `null` in that case.
...
> **Edit (14 may 2018):** Google implemented this solution directly in support library 28.0.0 and AndroidX 1.0.0. All fragments now provide an additional `getViewLifecycleOwner()` method just like in the above sample, so you donвЂ™t need to implement it yourself.
...
The layout declares a variable of type `Page1ViewModel` and binds the `TextView`вЂ™s `android:text` property directly to the `LiveData` field. When the `LiveData` is updated, the `TextView` will immediately reflect its new value.
...
The `ViewModel` will always reflect the latest visual state of the fragment, even when no view hierarchy is currently attached to it. **Each time a view hierarchy is created through a Binding class, it will register itself as a new observer on the LiveData instances, so we donвЂ™t have to manage any observer manually anymore** and we got rid of the problem.
```class Page1Fragment : Fragment() {
...
Also, when a fragment is detached, all its child fragments are detached as well. When in doubt, itвЂ™s better to assume that any fragment will eventually be detached at some point and properly handle this case from day 1.
...
DonвЂ™t hesitate to discuss this in the comments section and please share if you like.
""".trimIndent()
...
So I thought IвЂ™ll give it a try and share my findings. IвЂ™ll show you how to pass data using this plugin and also what types of data is supported.

_**Note:** At the time of writing most recent release of safe args Gradle plugin was **alpha04**. IвЂ™ll try to update this post when things change in future._
...
**Note:** When you select argument type it will generate this xml code: `app:type=вЂќstringвЂќ`. If you try to rebuild project, Android Studio will give you an **error:**

> The вЂtypeвЂ™ attribute used by argument вЂtitleвЂ™ is deprecated. Please change all instances of вЂtypeвЂ™ in navigation resources to вЂargTypeвЂ™.

ItвЂ™s self explanatory enough so you know that you should change it to `app:argType=вЂќstringвЂќ` manually in xml. ItвЂ™s because in plugin alpha04 release theyвЂ™ve changed it. ThereвЂ™s already an issue on issue tracker for this so it should be fixed in upcoming releases.

When youвЂ™re done, switch to text tab and you should see something like this in your destination:
```<fragment ...>
<argument
    android:name="title"
...
First class is used for inserting data. It has name after the direction from which you send data and ends with **вЂDirectionsвЂ™** e.g. *HomeFragmentDirections*. Second one is used to retrieve data at target direction and its name ends with **вЂArgsвЂ™** e.g. *DetailFragmentArgs*.

If you look at the generated classes, youвЂ™ll see theyвЂ™re using Bundle and properties matching the arguments defined in our xml. We can access these properties through getters and setters.

To send data from destination you need to obtain an instance of generated class which is named after **action** that you created for navigating to another destination (in this example itвЂ™s called *ActionDetail*). After that just insert your data using a setter method and pass *actionDetail* instance as a parameter in your *NavController.navigate* call.
```val actionDetail = HomeFragmentDirections.ActionDetail()
actionDetail.setTitle(item)
Navigation.findNavController(view).navigate(actionDetail)```
Receiving data in target destination depends on it being an activity or fragment. If youвЂ™re retrieving data in **activity** you would get the bundled data from extras in *onCreate* method:
...
And thatвЂ™s all. YouвЂ™ve successfully transferred your data using safe args.
...
It worked without problems. Then I found this issue on issue tracker where people asked for support for more types. So it looks like theyвЂ™ve already done it for **boolean, long** and **float**. Using **char** and **double** as an argument type didnвЂ™t work though. Compile error was: `part вЂdoubleвЂ™ is keyword.`
...
> An argument with a type of вЂњreferenceвЂќ is a reference to an Android resource i.e., with values like @string/value in XML or R.string.value in code.

There IвЂ™ve also learned that since safe args plugin alpha03 (I was using alpha04 at the time) it also supports **Parcelable**.

You should use `@null` as default value and you have to also add `app:nullable=вЂќtrueвЂќ` to allow nullable types because arguments are considered non-null by default and Android Studio will give you an error.
...
```<argument
 android:name=вЂќitemвЂќ
 android:defaultValue=вЂќ@nullвЂќ
 app:nullable=вЂќtrueвЂќ
 app:argType=вЂќcom.vepe.navigation.model.ItemвЂќ />```
Where Item is a simple data class in my model layer implementing Parcelable interface. If youвЂ™re not familiar with `@Parcelize` annotation, look [here](https://proandroiddev.com/parcelable-in-kotlin-here-comes-parcelize-b998d5a5fcac#70a6).
```@Parcelize
data class Item(val index: Int, val title: String, val value: Double): Parcelable```
_**Note:** You have to use a full class name. Using `app:argType=вЂќItemвЂќ` will result in compile error because generated classes wonвЂ™t be able to find and import your class._

***

### Conclusion
It looks like safe args plugin currently supports more types than Android Studio 3.2 lets you know about. ThatвЂ™s understandable of course, since the studio is still in beta (beta5 at the time of writing). I guess theyвЂ™ll update the Navigation editor UI to match this in future release.

But itвЂ™s nice to know that weвЂ™re already able to pass values of type long, float, boolean and also Parcelable objects and not just integer, string and reference.
...
ThatвЂ™s all for now. I hope you find information in this post useful. If you have any suggestions or questions, feel free to leave a comment below. рџ‰

Also this is my first post so every single clap will be really appreciated. рџ‘Џвќ¤пёЏ
""".trimIndent()
...
Since the image itself is self-explanatory, I wonвЂ™t go in details. More information can be found [here](https://guides.codepath.com/android/creating-and-using-fragments).
#### The lifecycle of each view hierarchy
This was something interesting which I never knew. The lifecycle of a FragmentвЂ™s view is:
...
> You can register an observer paired with an object that implements the `LifecycleOwner` interface. **This relationship allows the observer to be removed when the state of the corresponding `Lifecycle` object changes to `DESTROYED`.** This is especially useful for activities and fragments because they can safely observe `LiveData` objects and not worry about leaksвЂ”activities and fragments are instantly unsubscribed when their lifecycles are destroyed.
...
* When the user presses back on `FragmentB` , `FragmentA` goes through `onCreateView()` в†’ `onViewCreated` в†’ `onActivityCreated`
...
If we observe in `onCreate` and Fragment's view is recreated (visible в†’ backstack в†’ comes back), we have to update the values from `ViewModel` manually. This is because `LiveData` will not call the observer since it had already delivered the last result to that observer.
""".trimIndent()
...
IвЂ™m going to show you how navigation component can simplify your everyday Android stuff like navigating when clicking on adapter item or through navigation drawer.

IвЂ™ve also created a simple app in Kotlin where youвЂ™ll find everything mentioned in this article. So if you have trouble with something, you can look at the whole code at this repo.

***

### First things first
If youвЂ™re completely new to Navigation component, you can go through [this](https://proandroiddev.com/android-navigation-arch-component-a-curious-investigation-3e56e24126e1) post to learn how to create navigation graph with the use of Navigation editor. Or [this](https://medium.com/google-developer-experts/android-navigation-components-part-1-236b2a479d44) post series to also help structure your app better. And itвЂ™s always good practice to look at the [documentation.](https://developer.android.com/topic/libraries/architecture/navigation/navigation-implementing)

ThereвЂ™s also a [codelab](https://codelabs.developers.google.com/codelabs/android-navigation/#0) which is a great way to start if youвЂ™re not familiar with this component but you want to learn by coding.

Ready? OK, letвЂ™s start by taking a closer look at the Navigation Component.

***

### Navigation Component main players
Before diving into code we should have some knowledge about classes and interfaces that weвЂ™re going to work with. [Documentation](https://developer.android.com/reference/androidx/navigation/package-summary) mentions three main components:

* **NavGraph** вЂ” a collection of destinations. It can be inflated from layout file or created programmatically. You can have multiple navigation graphs in your application and they can be also nested.
* **NavHost** вЂ” an interface serving as a container that hosts the NavController.
* **NavController** вЂ” class managing navigation within NavHost by interacting with NavGraph.

Other classes youвЂ™ll use include:

**NavHostFragment** вЂ” implementation of NavHost for creating fragment destinations. It has its own NavController and navigation graph. Typically you would have one NavHostFragment per activity.

**Navigation** вЂ” helper class for obtaining NavController instance and for connecting navigation to UI events like a button click.

**NavigationUI** вЂ” class for connecting app navigation patterns like drawer, bottom navigation or actionbar with NavController.

***

### LetвЂ™s quickly refresh the basics
LetвЂ™s say youвЂ™ve created navigation graph XML file and youвЂ™ve created a fragment in your activity layout file.
...
```<fragment
    android:layout_width=вЂќmatch_parentвЂќ
    android:layout_height=вЂќmatch_parentвЂќ
    android:id=вЂќ@+id/nav_hostвЂќ
    android:name=вЂќandroidx.navigation.fragment.NavHostFragmentвЂќ
    app:navGraph=вЂќ@navigation/nav_graphвЂќ
    app:defaultNavHost=вЂќtrueвЂќ //to intercept with system Back button
 />```
...
If you donвЂ™t have `NavController` you can obtain it in multiple ways:
...
where first parameter is action id or destination id and second is **bundle** for passing data between destinations. ThereвЂ™s also one argument version of this function when you donвЂ™t need to send data.
...
In [this](https://issuetracker.google.com/issues/79672220) issue someone from Google recommended using a **shared ViewModel.** For activities we should probably continue using the old way, because you canвЂ™t share ViewModel between activities like you can with fragments.
...
nav_view.setNavigationItemSelectedListener { menuItem ->
 when (menuItem.itemId) {
            R.id.nav_first -> go somewhereвЂ¦
            R.id.nav_second -> go somewhere elseвЂ¦
 }
...
Navigation component makes it easier to react to click events on items in `RecyclerView`. The old way would be to create a listener interface that the activity/fragment implements and pass its instance to adapter via constructor. Then pass it to `ViewHolder` so you can call the listenerвЂ™s method inside itemвЂ™s on click listener implementation.

With Navigation component you donвЂ™t need to create an interface nor pass a listener instance to adapter if youвЂ™re only interested in navigating to another screen. Just set `onClickListener` on itemвЂ™s view as follows:
...
LetвЂ™s say user wants to navigate from Home screen to Profile but hasnвЂ™t logged in yet. Instead of Profile, Login screen is shown. After a successful login user is taken to the Profile screen. If he navigates back, he should be taken straight to the Home screen. [Documentation](https://developer.android.com/topic/libraries/architecture/navigation/navigation-conditional) suggests that
...
If youвЂ™re implementing **login flow** using a **separate activity**, simply call `activity.finish()` before navigating from login to another activity.

This way the login activity wonвЂ™t stay in the stack and navigating up wonвЂ™t show it. This is useful also with `onboarding flow` at app launch, because you donвЂ™t want to let user return to it by pressing the back button.

***

### Common destinations and global actions
Common destination is a screen to which can user navigate from multiple parts of the app. For such cases we can use **global actions.** Unlike normal actions, they are defined on the same level as destinations вЂ” inside the root element of graph called `navigation`. They have an **id** and **target** destination. Global action can be used to navigate to its target destination from any other destination in the same navigation graph (nested graph also).
...
As your app becomes more filled with features you may consider putting some destinations from your navigation graph to a **nested** graph. ItвЂ™s basically a graph within your **main/root** graph. This is useful if you have a setup flow that consists from multiple screens or an onboarding flow.
...
Also it enables you to **reuse** the nested flow in your app. ThatвЂ™s because a nested graph has an **id** which is used as the only access point for navigation from the root graph. You wonвЂ™t be able to navigate to specific destination in nested graph from the root graph.
```<navigation android:id="@+id/main" ...>
...
There are still some topics that I havenвЂ™t covered in this article. **Deep linking** is well described in [this article](https://medium.com/google-developer-experts/android-navigation-components-part-3-19554ec9ae83). Navigation Component **testing** isnвЂ™t really covered in docs yet, but Maria Neumayer covers it briefly [here}(https://medium.com/a-problem-like-maria/a-problem-like-navigation-e9821625a70e).

If you have any suggestions or questions, feel free to leave a comment below. DonвЂ™t forget to leave рџ‘Џ if you liked the post. It will be appreciated. вќ¤пёЏ
""".trimIndent()
...
### Setup
To take advantage of the latest VM component, declare the most recent [Lifecycle dependencies](https://developer.android.com/jetpack/androidx/releases/lifecycle#declaring_dependencies) in the *build.gradle (Module: app)* file for `lifecycle-viewmodel-ktx` and `lifecycle-livedata-ktx`. The `jvmTarget` also needs to be defined in order to implement the `by viewModels` syntax weвЂ™ll use below.
```android {
  ...
...
* Use `by viewModels` when the activity/fragment creating the VM will be the only activity/fragment accessing the VMвЂ™s data.

Behind the scenes `viewModels` is an extension function applied to either an activity/fragment that returns a VM with a lifecycle tied to that activity/fragment. A VM factory may optionally be defined which can be used to pass parameters/arguments as weвЂ™ll see below.

* Use by `activityViewModels` in fragments, when the fragment is sharing data/communicating with another activity/fragment.

This is useful for sharing information between views. `activityViewModels` is an extension function applied to a fragment that returns a fragmentвЂ™s parentвЂ™s activityвЂ™s VM. A VM factory may be defined here as well.
...
* Use a pattern that allows the readability of the values in the VM, such as LiveDataвЂ™s transform or CoroutineвЂ™s collect.
...
* When the creating activity/fragment is finished/detached, VMвЂ™s `onCleared` is called
...
If an initialization process is tied to an activity/fragment lifecycle event, it may unintentionally occur if the activity/fragment is recreated. When the process runs in the VMвЂ™s `init{..}` function, it will be tied to the VM lifecycle.
...
#### CanвЂ™t we just use AndroidViewModel (AVM)?
...
* DonвЂ™t load resources, instead pass resource ids, and load in view. See: [AndroidViewModel antipattern](https://medium.com/androiddevelopers/locale-changes-and-the-androidviewmodel-antipattern-84eb677660d9)
...
Imagine weвЂ™ve got a table that contains information about tags, defined with the following `CREATE TABLE` statement:
```CREATE TABLE tags (
    title TEXT,
    description TEXT,
    created TEXT
);```
![](https://miro.medium.com/max/700/1*5wzoyIgWMmqCn054K1z1TA.png "Table doesnвЂ™t explicitly specify a PRIMARY KEY")
This table doesnвЂ™t explicitly specify a `PRIMARY KEY` column, so `rowid` is used instead.
When we run `SELECT title FROM tags WHERE rowid = 6` SQLite does a binary search, first looking at the middle record with `rowid = 9/2 = 4`, realizes thatвЂ™s too low, and tries `rowid = (9-4)/2 + 4 = 2 + 4 =6` and bingo! That took 2 steps, and is a lot faster than iterating through each record until it finds the 6th record (which clearly wouldвЂ™ve taken 6 steps).
But wait, why couldnвЂ™t SQLite just immediately jump to the 6th record? Well, even though the `rowid` is auto-incrementing and unique, we couldвЂ™ve deleted rows in the table and thereby created gaps where the 6th record might not be the record with `rowid = 6`.
The algorithms SQLite uses to determine how to execute a query are grouped up into a module called the вЂњQuery PlannerвЂќ, and performing that determination is called вЂњquery planningвЂќ.
### Indexes
So far, weвЂ™ve only been talking about primary keys. What if we donвЂ™t necessarily know the `rowid` for the tag we need, but instead wanted to search for the вЂњKotlinвЂќ tag:
...
If you prefix the query from above with `EXPLAIN QUERY PLAN` you can verify that SQLite will need to perform a full scan of the table just to find the вЂњKotlinвЂќ tag:
...
Now, when we run `SELECT * FROM tags WHERE title = "Kotlin"`, SQLite can use the `tag_titles` index to perform a binary search on the `title` values to determine that the record weвЂ™re looking for has `rowid = 5` and can then use that value in a second binary search against the actual `tags` table to return the full record.
![](https://miro.medium.com/max/1000/1*AIotEUXh87wi61HWL1f0sQ.png "One binary search to find the rowid for вЂњKotlinвЂќ, another to find the data.")
In short: using the ```tag_titles``` index brings the queryвЂ™s performance from O(n) to O(log n + log n) = O(log n) which is similar to if we were to query on the primary key.
...
### вЂњCovering IndexesвЂќ
In situations where you tend to do a lot of querying for subsets of your tableвЂ™s data, you can utilize what are called вЂњCovering IndexesвЂќ to eliminate that second binary search from the query planning operation.
...
We saw that with the index described in the last section: one binary search is done to find the `rowid` of the record where the title is вЂњKotlinвЂќ, and a second is done to find the remainder of the data that was required. LetвЂ™s create a new index which includes the `description` as well as the `title`:
...
Now when you run the query, SQLiteвЂ™s query planner is smart enough to recognize that we only need data which is contained within the index and can return that data immediately after the index search without resorting to looking at the actual records in the `tags` table.
```EXPLAIN QUERY PLAN 
SELECT title, description FROM tags WHERE title = "Kotlin";```
![](https://miro.medium.com/max/700/1*XY4g5qb2cauqf4LmTNcZXQ.png "EXPLAIN QUERY PLAN result")
The `tag_title_desc` is considered a вЂњcovering indexвЂќ because it covers all the fields needed to satisfy the query.
> *One O(log n) operation is better than two, especially when the data set is huge.
> - some smart person*
Side Note: Although the `tag_title_desc` defines `description` as a field on the index, it wonвЂ™t help you if youвЂ™re trying to query the `tags` table by description. This is because the index is sorted by `title` first, and `description` second, and thereвЂ™s no good way to binary-search the descriptions using an index like that. If we need that functionality, we should consider adding a new index on the `description` field.
...
Here, SQLite doesnвЂ™t need to build up the temporary data structure and can simply iterate through the `tag_titles` index. When fetching the data for the result set it will perform binary searches to retrieve each `tags` record.
...
Because indexes are sorted data structures and their benefit comes from how binary search works, itвЂ™s important to ensure that your indexed columns have what is called вЂњhigh cardinalityвЂќ. All this means is that the indexed data has a lot of uniqueness.
...
#### The Costs
Additionally, itвЂ™s important to remember two important costs to creating and maintaining indexes on your tables:
...
* ___Insert/Update/Delete Performance.__ When you insert, update, or delete records in your indexed tables, itвЂ™s important that the indexes are kept up to date as well as in the correct order. Each of these operations will take slightly longer when there are indexes on the table than when there are no indexes. If youвЂ™re going to be doing a lot of updates, consider dropping your indexes before the operation and re-creating them afterwards.
### Conclusion
Indexes are a powerful tool at your disposal to boost performance of your applicationвЂ™s data retrieval. They help with querying and sorting the data, and when covering indexes are used correctly can make lookups incredibly fast. Covering indexes take the whole idea to the next level.
Indexes also come with some overhead: they will use additional space, and can increase the time it takes to insert or update records because SQLite needs to keep the indexes up to date. ItвЂ™s important to weigh the pros and cons before you add an index to your table.
This post touches on the basics of indexes in SQLite, but you can read a whole lot more about how SQLiteвЂ™s query planner works and its use of indexes in the [official documentation](https://sqlite.org/queryplanner.html). Additionally, for the sake of making this post more approachable, I purposefully glossed over how the indexes are built and maintained. You can read more about how SQLite (and other databases) maintain indexes by learning about [B-trees](https://en.wikipedia.org/wiki/B-tree).
Finally, a lot of what we discussed in this post can be applied to how indexes work in other database systems.
If you enjoyed this post, please tap that heart button to recommend it to your followers. Also, you should check out the other posts in my series: вЂњSqueezing Performance from SQLiteвЂќ:
""".trimIndent()

val articleContent7: String = """
You donвЂ™t need to tell me how many times you have written something like this:
...
LetвЂ™s instantiate a `NumberFormatter` and set its `numberStyle` property:
```let numberFormatter = NumberFormatter()
numberFormatter.numberStyle = .currency```
As we can see, we specified that we want a currency representation of our values.
LetвЂ™s quickly test some values:
...
numberFormatter.numberStyle = .currency
numberFormatter.currencySymbol = "в‚¬"
...
LetвЂ™s change the `numberStyle` to `.spellOut`:
```let numberFormatter = NumberFormatter()
...
As per the Apple Documentation: вЂњAn enumeration defines a common type for a group of related values and enables you to work with those values in a **type-safe way within your codeвЂќ.**
...
SwitchвЂ™s statement also **follows the multi-way selection.** Only one variable is accessed at a particular time based on the specified condition. The default case in a switch statement is used to **trap unspecified cases.**

> default case in switch is not required since weвЂ™ve covered all the enum cases in the above code.

In most cases, enum will not have all the values in the Switch statement, so it is necessary to give the default case in the switch statement. Let us see the exampleвЂ¦
#### Iterating over Enumeration Cases
In the example above, we can write `Bikes.allCases` to access a collection that contains all of the cases of the `Bikes` enumeration. We can use `allCases` like any other collectionвЂ”the collectionвЂ™s elements are instances of the enumeration type, so in this case, theyвЂ™re `Bikes` values.
...
#### Recursive Enums
LetвЂ™s create an enum called, `ArithmeticExpression`. It contains three cases with associated types. Two of the cases contain its own enum type, `ArithmeticExpression`. The `indirect` keyword tells the compiler to handle this enum case indirectly.
...
#### Properties in Enums
We canвЂ™t add actually **stored properties to an enum, but we can create computed properties.** The value of computed properties can be based on the enum value or enum associated value.
LetвЂ™s create an enum called `Device`. It contains a computed property called `year`, which returns the first appearance of that device.
""".trimIndent()
...
One more advantage of native over cross-platform development is performance. In the technical world, you can encounter вЂњcross-platform apps are slowвЂќ stereotypes. We decided to test if itвЂ™s true and to what extend cross-platform apps are slower than native.
***
...
***
### CPU-intensive test (GaussвЂ“Legendre algorithm) for iOS
![Memory-intensive test (GaussвЂ“Legendre algorithm) for iOS](https://miro.medium.com/max/1400/1*Ey1f_IcZiwM75XGKY83nVw.jpeg "Memory-intensive test (GaussвЂ“Legendre algorithm) for iOS")
#### iOS
...
### CPU-intensive test (GaussвЂ“Legendre algorithm) for Android
![Memory-intensive test (GaussвЂ“Legendre algorithm) for Android](https://miro.medium.com/max/1400/1*C5bM9KdtdjHpftFxgBn-UA.jpeg "Memory-intensive test (GaussвЂ“Legendre algorithm) for Android")
#### Android
...
* GaussвЂ“Legendre & Borwein algorithms of calculating Pi numbers were used. The Pi number has been calculated 100 times with 10 million digits precision.
* GaussвЂ“Legendre is a more memory-intensive algorithm in comparison with Borwein, but Borwein is more CPU-intensive.
* [Source code](https://github.com/nazarcybulskij/Mobile_Bechmarks_)
***
### Key takeaways
* In summary, not all cross-platform apps are slow. WhatвЂ™s more than that, Flutter apps have higher performance than Swift apps.
...
val articleItems = Array(10) {
    when (it) {
        0 -> ArticleData(
            ...
            description = "LetвЂ™s say that we need to draw a rounded corner background on text, supporting the following cases",
        ...
        2 -> ArticleData(
            ...
            title = "Using Safe args plugin вЂ” current state of affairs",
